...
namespace DSEV.Schemas
{
    ...
    public partial class 장치통신
    {
        ...
        public Boolean PLC에러상태신호 { get => 신호읽기(정보주소.PLC에러상태); set => 정보쓰기(정보주소.PLC에러상태, value); }
        public Boolean PLC에러리셋신호 { get => 신호읽기(정보주소.PLC에러리셋); set => 정보쓰기(정보주소.PLC에러리셋, value); }
        ...
        {
            int lowerWord = Data & 0xFFFF;    // 하위 16비트
            int upperWord = (Data >> 16) & 0xFFFF; // 상위 16비트

            Int32 NewAddress_int = Int32.Parse(Address.Substring(1));
            Int32 NextNewAddress_int = NewAddress_int + 1;


            오류코드 = PLC.SetDevice(Address, lowerWord);
            오류코드 = PLC.SetDevice("W" + NextNewAddress_int.ToString(), upperWord);
            //if (upperWord != 0) 오류코드 = PLC.SetDevice("W" + NextNewAddress_int.ToString(), upperWord);
